...
func TestRecordingVerbsStayAbsent(t *testing.T) {
	server := shellServer(t, &scriptRunner{})
	for _, method := range []string{"shell/tap", "shell/history"} {
		_, rpcErr := shellCall(t, server, "test-token", method, `{"name":"work"}`)
		if rpcErr == nil || rpcErr.Code != CodeMethodNotFound {
			t.Fatalf("%s must be unavailable: %+v", method, rpcErr)
		}
	}
}
