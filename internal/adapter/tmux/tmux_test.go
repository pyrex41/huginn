...
func (f *fakeRunner) Run(ctx context.Context, args ...string) ([]byte, error) {
	...
	if args[0] == "display-message" && args[len(args)-1] == "#{pane_id}" {
		return []byte("%4\n"), nil
	}
...
func TestSendLiteralThenEnter(t *testing.T) {
	...
	if strings.Join(sends[0], " ") != "send-keys -t %4 -l -- make -j4 ; send-keys -t %4 Enter" {
...
func TestKeysAreNames(t *testing.T) {
	...
	found := false
	for _, c := range f.calls {
		if strings.Join(c, " ") == "send-keys -t %4 C-c Enter" {
			found = true
		}
	}
	if !found {
...
func TestScreenBounds(t *testing.T) {
	...
	for _, count := range []int{-1, MaxScreenLines + 1} {
		if _, err := adapter.Screen(context.Background(), "work", "", count); !errors.Is(err, ErrBadLines) {
			t.Fatalf("lines=%d: %v", count, err)
		}
	}
	if len(runner.calls) != 0 {
		t.Fatal("invalid bounds must fail before running tmux")
	}
	var output boundedOutput
	if _, err := output.Write(make([]byte, MaxOutputBytes+1)); !errors.Is(err, ErrOutputLimit) || output.buffer.Len() != 0 {
		t.Fatalf("oversized output: bytes=%d err=%v", output.buffer.Len(), err)
	}
}
...
func TestExecOutputBound(t *testing.T) {
	...
	if _, err := (ExecRunner{Binary: path}).Run(context.Background()); !errors.Is(err, ErrOutputLimit) {
		t.Fatalf("exec must bound output: %v", err)
	}
}

func TestScrollbackGenMatchesVisibleScreen(t *testing.T) {
	runner := &fakeRunner{screens: []string{"history\nprompt\n", "prompt\n", "prompt\n", "changed\n"}}
	adapter := NewWithRunner(runner)
	screen, err := adapter.Screen(context.Background(), "work", "", 100)
	if err != nil || screen.Gen != Digest("prompt\n") {
		t.Fatalf("screen=%+v err=%v", screen, err)
	}
	if _, err := adapter.Send(context.Background(), "work", "", "x", true, screen.Gen); err != nil {
		t.Fatal(err)
	}
	for _, args := range runner.calls {
		if args[0] == "capture-pane" || args[0] == "send-keys" {
			if !strings.Contains(strings.Join(args, " "), "-t %4") {
				t.Fatalf("operation must pin the pane ID: %v", args)
			}
		}
	}
}

func TestKeysRefuseCommandSeparator(t *testing.T) {
	adapter := NewWithRunner(&fakeRunner{})
	if _, err := adapter.Keys(context.Background(), "work", "", []string{";", "kill-server"}, ""); !errors.Is(err, ErrBadKey) {
