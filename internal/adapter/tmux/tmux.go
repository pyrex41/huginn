...
func (r ExecRunner) Run(ctx context.Context, args ...string) ([]byte, error) {
	...
	if err != nil {
		...
		if errors.As(err, &ee) {
			return nil, &ExitError{Args: args, Code: ee.ExitCode(), Stderr: stderr.buffer.String()}
		}
		return nil, err
	}
...
const MaxOutputBytes = 128 << 10
const MaxScreenLines = 5000
...
func (b *boundedOutput) Write(data []byte) (int, error) {
	if b.buffer.Len()+len(data) > MaxOutputBytes {
		b.exceeded = true
...
var (
	...
	ErrBadLines    = errors.New("tmux: lines must be between 0 and 5000")
	ErrOutputLimit = errors.New("tmux: output exceeds 128 KiB; request fewer lines or fewer sessions")
)
...
type Screen struct {
	...
	// Gen hashes visible text only, independent of requested scrollback.
	Gen string `json:"gen"`
}
...
func NewForSession(socket, sessionID string) *Adapter {
	adapter := New(socket)
...
func NewWithRunner(r Runner) *Adapter {
	host, _ := os.Hostname()
	return &Adapter{runner: r, host: host}
}
...
func (a *Adapter) List(ctx context.Context) ([]Shell, error) {
	sessionArgs := []string{"list-sessions", "-F", sessionFormat}
	windowArgs := []string{"list-windows", "-a", "-F", windowFormat}
	paneArgs := []string{"list-panes", "-a", "-F", paneFormat}
	if a.sessionID != "" {
		sessionArgs = []string{"display-message", "-p", "-t", a.sessionID, "-F", sessionFormat}
		windowArgs = []string{"list-windows", "-t", a.sessionID, "-F", windowFormat}
		paneArgs = []string{"list-panes", "-s", "-t", a.sessionID, "-F", paneFormat}
	}
	out, err := a.runner.Run(ctx, sessionArgs...)
	if err != nil {
		if noServer(err) {
			return []Shell{}, nil
		}
	...
	wout, err := a.runner.Run(ctx, windowArgs...)
	...
	pout, err := a.runner.Run(ctx, paneArgs...)
	if err != nil && !noServer(err) {
		return nil, err
	}
...
func (a *Adapter) Screen(ctx context.Context, name, pane string, lines int) (Screen, error) {
	if lines < 0 || lines > MaxScreenLines {
		return Screen{}, ErrBadLines
	}
	target, err := a.resolvePane(ctx, name, pane)
	if err != nil {
		return Screen{}, err
	}
...
func (a *Adapter) capture(ctx context.Context, target string, lines int) (Screen, error) {
	...
	if lines > 0 {
		visible, err := a.capture(ctx, target, 0)
		if err != nil {
			return Screen{}, err
		}
		scr.Gen, scr.Cursor = visible.Gen, visible.Cursor
		return scr, nil
	}
...
func (a *Adapter) Send(ctx context.Context, name, pane, text string, enter bool, expectGen string) (Sent, error) {
	...
	return a.input(ctx, name, pane, expectGen, func(target string) error {
		args := []string{"send-keys", "-t", target, "-l", "--", argument(text)}
		if text != "" {
			if enter {
				args = append(args, ";", "send-keys", "-t", target, "Enter")
			}
		} else {
...
func (a *Adapter) NewShell(ctx context.Context, name, cwd, command string) (Shell, error) {
	if a.sessionID != "" {
		return Shell{}, errors.New("creating sessions is unavailable on a shared session")
	}
	if err := ValidateName(name); err != nil {
		return Shell{}, err
	}
	args := []string{"new-session", "-d", "-s", argument(name)}
	if cwd != "" {
		args = append(args, "-c", argument(cwd))
	}
	if command != "" {
		args = append(args, "--", argument(command))
	}
...
func (a *Adapter) Kill(ctx context.Context, name string) error {
	if a.sessionID != "" {
		return errors.New("killing sessions is unavailable on a shared session")
	}
	if err := ValidateName(name); err != nil {
		return err
	}
	if _, err := a.runner.Run(ctx, "kill-session", "-t", argument(exact(name))); err != nil {
		return mapErr(err)
	}
	return nil
}
...
func (a *Adapter) target(ctx context.Context, name, pane string) (string, error) {
	...
	sessionTarget := exact(name)
	if a.sessionID != "" {
		if _, err := a.Get(ctx, name); err != nil {
			return "", err
		}
		sessionTarget = a.sessionID
	}
	if pane == "" {
...
func (a *Adapter) resolvePane(ctx context.Context, name, pane string) (string, error) {
	target, err := a.target(ctx, name, pane)
	if err != nil || strings.HasPrefix(target, "%") {
		return target, err
	}
	output, err := a.runner.Run(ctx, "display-message", "-p", "-t", target, "-F", "#{pane_id}")
	if err != nil {
		return "", mapErr(err)
	}
...
func ValidateKey(k string) error {
	if k == "" || len(k) > 32 || strings.HasPrefix(k, "-") || strings.Contains(k, ";") {
		return ErrBadKey
	}
	for _, r := range k {
		if r <= 0x20 || r == 0x7f {
