...
var shellExpectProp = map[string]any{"type": "string", "description": "Visible-text digest from shell_screen. Best-effort stale-screen check, not a lock or atomic revision."}
		...
		},
		{
			...
			"inputSchema": map[string]any{
				...
				"properties": map[string]any{
					...
					"name":    shellNameProp,
					"pane":    shellPaneProp,
					"lines":   map[string]any{"type": "integer", "minimum": 0, "maximum": 5000, "description": "Scrollback lines before the visible screen; output capped at 128 KiB"},
				},
...
func (s *server) shellList(ctx context.Context, args map[string]any) map[string]any {
	...
	out := fanOut(targets, func(target string) shellMachineResult {
		result := shellMachineResult{Machine: target}
		...
		if err := json.Unmarshal(raw, &parsed); err != nil {
			result.Error = "unparseable reply from " + target
			return result
		}
		result.Shells, result.Total, result.NextCursor = parsed.Shells, parsed.Total, parsed.NextCursor
		return result
	})
